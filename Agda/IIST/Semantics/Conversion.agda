...
module _ where
  ...
  >>=-just∘ : ∀ (m : Maybe X) (f : X → Y) → (m >>= λ x → just (f x)) ≡ (f <$> m)
  >>=-just∘ nothing f = refl
  >>=-just∘ (just x) f = refl
  ...
  >>=-eqᵣ : ∀ (m : Maybe X) {f g : X → Maybe Y} (p : ∀ x → f x ≡ g x)
    → (m >>= f) ≡ (m >>= g)
  >>=-eqᵣ (just x) p = p x
  >>=-eqᵣ nothing p = refl
  ...
  ⊗ₛ-eatₛ-dist (S.next f) (S.yield w g) xzs =
    let xs , zs = unzip xzs in
    begin
      S.eat (S.cast _ (f S.⊗ w S.▸ g)) xzs
    ≡⟨ S.≈-eat S.≈-cast xzs ⟩
      S.eat (f S.⊗ w S.▸ g) xzs
    ≡⟨ ⊗-eat-dist f (w S.▸ g) xzs ⟩
      (| List.zip (S.eat f xs) (S.eat (w S.▸ g) zs) |)
    ≡⟨ {!   !} ⟩
      (| List.zip (S.eat f xs) ((w ∷_) <$> S.eat g zs) |)
    ∎
  ⊗ₛ-eatₛ-dist (S.yield y f) (S.next g) xzs =
    let xs , zs = unzip xzs in
    begin
      S.eat (y S.▸ f S.⊗ g) xzs
    ≡⟨ ⊗-eat-dist (y S.▸ f) g xzs ⟩
      (| List.zip (S.eat (y S.▸ f) xs) (S.eat g zs) |)
    ≡⟨ {!   !} ⟩
      (| List.zip ((y ∷_) <$> S.eat f xs) (S.eat g zs) |)
    ∎
  ⊗ₛ-eatₛ-dist (S.yield y f) (S.yield w g) xzs =
    ...
    ≡⟨ cong (((y , w) ∷_) <$>_) (⊗-eat-dist f g xzs) ⟩
      ((y , w) ∷_) <$> (| List.zip (S.eat f xs) (S.eat g zs) |)
    ≡⟨ <*>-map₂ ((y , w) ∷_) List.zip (S.eat f xs) _ ⟩
      (| (λ ys ws → List.zip (y ∷ ys) (w ∷ ws)) (S.eat f xs) (S.eat g zs) |)
    ≡⟨ map-<*>₂ List.zip (y ∷_) (w ∷_) (S.eat f xs) _ ⟨
      (| List.zip ((y ∷_) <$> S.eat f xs) ((w ∷_) <$> S.eat g zs) |)
    ∎
...
module _ where
  ...
  zip-▸-∷∞ : ∀ (f : S.IST X Y d) (g : S.IST Z W 0) w xzs
    → let xs , zs = unzipₗ xzs , unzipᵣ xzs in
      zip (S.eat∞ f xs) (S.eat∞ (w S.▸ g) zs) ≈ zip (S.eat∞ f xs) (w ∷ delay (S.eat∞ g zs))
  zip-▸-∷ₛ∞ : ∀ (s : S.Step X Y d) (t : S.Step Z W 0) w (xzs : ∞Colist⊥ (X × Z))
    → let xs , zs = delay (unzipₗ (force xzs)) , delay (unzipᵣ (force xzs)) in
      zip (S.eatₛ∞ s xs) (S.eatₛ∞ (w S.▸ₛ t) zs) ≈ zip (S.eatₛ∞ s xs) (w ∷ delay (S.eatₛ∞ t zs))
  zip-▸-∷∞ f g w [] = []
  zip-▸-∷∞ f g w ⊥ = ⊥
  zip-▸-∷∞ f g w ((x , z) ∷ xzs) = zip-▸-∷ₛ∞ (S.step f x) (S.step g z) w xzs
  zip-▸-∷ₛ∞ S.⊥ _ w xzs = ⊥
  zip-▸-∷ₛ∞ (S.next f) S.⊥ w xzs = {!   !}
  zip-▸-∷ₛ∞ (S.yield y f) S.⊥ w xzs =
    (y , w) ∷ λ where .force → {!   !}
  zip-▸-∷ₛ∞ (S.next f) (S.yield w' g) w xzs = {!   !}
  zip-▸-∷ₛ∞ (S.yield y f) (S.yield w' g) w xzs =
    (y , w) ∷ λ where .force → {!   !}
  ...
  ⊗-eat∞-dist f g ((x , z) ∷ xzs) = ⊗ₛ-eatₛ∞-dist (S.step f x) (S.step g z) xzs
  ⊗ₛ-eatₛ∞-dist S.⊥ _ xzs = ⊥
  ⊗ₛ-eatₛ∞-dist (S.next f) S.⊥ xzs = ≈-sym zip-⊥ᵣ
  ⊗ₛ-eatₛ∞-dist (S.yield y f) S.⊥ xzs = ⊥
  ⊗ₛ-eatₛ∞-dist (S.next f) (S.next g) xzs = ⊗-eat∞-dist f g (force xzs)
  ⊗ₛ-eatₛ∞-dist (S.next f) (S.yield w g) xzs =
    let xzs = force xzs in
    let xs , zs = unzipₗ xzs , unzipᵣ xzs in
    begin
      S.eat∞ (S.cast _ (f S.⊗ w S.▸ g)) xzs
    ≈⟨ S.≈-eat∞ S.≈-cast xzs ⟩
      S.eat∞ (f S.⊗ w S.▸ g) xzs
    ≈⟨ ⊗-eat∞-dist f (w S.▸ g) xzs ⟩
      zip (S.eat∞ f xs) (S.eat∞ (w S.▸ g) zs)
    ≈⟨ zip-▸-∷∞ f g w xzs ⟩
      zip (S.eat∞ f xs) (w ∷ delay (S.eat∞ g zs))
    ∎
  ⊗ₛ-eatₛ∞-dist (S.yield y f) (S.next g) xzs =
    let xzs = force xzs in
    let xs , zs = unzipₗ xzs , unzipᵣ xzs in
    begin
      S.eat∞ (y S.▸ f S.⊗ g) xzs
    ≈⟨ ⊗-eat∞-dist (y S.▸ f) g xzs ⟩
      zip (S.eat∞ (y S.▸ f) xs) (S.eat∞ g zs)
    ≈⟨ {!   !} ⟩
      zip (y ∷ delay (S.eat∞ f xs)) (S.eat∞ g zs)
    ∎
