...
-- open import Data.Product.Base using ( Σ-syntax; ∃-syntax; _×_; _,_; fst; snd )
...
-- zip-unzip : ∀ (xys : List (X × Y)) → zip (fst (unzip xys)) (snd (unzip xys)) ≡ xys
...
-- ≺-unzip₁ : _≺_ =[ fst ∘ unzip {A = X} {B = Y} ]⇒ _≺_
-- ≺-unzip₁ [] = []
-- ≺-unzip₁ (xy ∷ p) = fst xy ∷ ≺-unzip₁ p

-- ≺-unzip₂ : _≺_ =[ snd ∘ unzip {A = X} {B = Y} ]⇒ _≺_
-- ≺-unzip₂ [] = []
-- ≺-unzip₂ (xy ∷ p) = snd xy ∷ ≺-unzip₂ p

-- unzip-zip₁ : ∀ (xs : List X) (ys : List Y) → fst (unzip (zip xs ys)) ≺ xs
-- unzip-zip₁ [] _ = []
-- unzip-zip₁ (x ∷ xs) [] = []
-- unzip-zip₁ (x ∷ xs) (y ∷ ys) = x ∷ unzip-zip₁ xs ys

-- unzip-zip₂ : ∀ (xs : List X) (ys : List Y) → snd (unzip (zip xs ys)) ≺ ys
-- unzip-zip₂ [] _ = []
-- unzip-zip₂ (x ∷ xs) [] = []
...
--   with just ys ← f (unzip xzs .fst) in eq₁
--   with just ys' ← f (unzip xzs' .fst) | _ , ys'≺ys , refl ← f-inc (≺-unzip₁ xzs'≺xzs) eq₁
--   with just ws ← g (unzip xzs .snd) in eq₂
--   with just ws' ← g (unzip xzs' .snd) | _ , ws'≺ws , refl ← g-inc (≺-unzip₂ xzs'≺xzs) eq₂
--   rewrite sym (just-injective eq) =
...
--         | f-delay (unzip xzs .fst) eq₁
--         | g-delay (unzip xzs .snd) eq₂
...
--   with just xs ← f' (unzip yws .fst) in eq₁
--   with just ys' ← f xs in eq₁ | _ , ys'≺ys , refl ← f-inv-f' eq₁
--   with just zs ← g' (unzip yws .snd) in eq₂
--   with just ws' ← g zs in eq₂ | _ , ws'≺ws , refl ← g-inv-g' eq₂
--   rewrite sym (just-injective eq)
--   with just ys'' ← f (unzip (zip xs zs) .fst) | _ , ys''≺ys' , refl ← f-inc (unzip-zip₁ xs zs) eq₁
--   with just ws'' ← g (unzip (zip xs zs) .snd) | _ , ws''≺ws' , refl ← g-inc (unzip-zip₂ xs zs) eq₂ =
--     zip ys'' ws'' ,
...
-- ... | nothing = refl
-- ... | just ys = F∘I≡B e ys
-- F∘I≡B (e `⊗ e') xzs rewrite F∘I≡B e (unzip xzs .fst) | F∘I≡B e' (unzip xzs .snd) =
--   refl
...
-- ... | nothing = refl
-- ... | just ys = B∘I≡F e' ys
-- B∘I≡F (e `⊗ e') xzs rewrite B∘I≡F e (unzip xzs .fst) | B∘I≡F e' (unzip xzs .snd) =
--   refl
